    ...
    def train(self):
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            for step, (batch_x, batch_y) in tqdm(enumerate(zip(batch_train_data, batch_train_label))):
                ...
                train_predict, train_accuracy, train_loss, _ = self.sess.run([self.model.train_predict, self.model.accuracy_train, self.model.loss, self.model.optimizer], feed_dict=feed_dict)
                train_accuracy_list.append(train_accuracy)
                train_loss_list.append(train_loss)
                train_label_list.extend([1.0]*len(train_predict))
            ...
            # 完成一个epoch的训练，输出训练数据的mean accuracy、mean loss
            logging.info('Test Epoch: %d , Acc: %.6f , Precision: %.6f , Recall: %.6f , F1: %.6f' % (epoch,
...
def load_dataset(data_path):
    ...
    for line in tqdm(open(data_path, encoding='UTF-8')):
        sentence_list = str(line).strip().replace('\n', '').split('\t')
        if len(sentence_list) != config.neg_doc_num+2:
            # logging.info('Load Data Error ... msg: {}'.format(sentence_list))  # 部分数据去除英文和数字后为空，不为neg_doc_num+2条
            continue
